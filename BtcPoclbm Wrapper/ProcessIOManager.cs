...
namespace ProcessReadWriteUtils {
    ...
    public class ProcessIOManager {
        ...
        private readonly StringBuilder streambuffer;
        ...
        public void StartProcessOutputRead() {
            ...
            if (runningProcess.StartInfo.RedirectStandardOutput) {
                stdoutThread = new Thread(ReadStandardOutputThreadMethod);
            ...
            // running process, and report text read via supplied events.
            if (runningProcess.StartInfo.RedirectStandardError) {
                stderrThread = new Thread(ReadStandardErrorThreadMethod);
        ...
        public void WriteStdin(string text) {
            ...
            if (runningProcess.StartInfo.RedirectStandardInput) {
        ...
        /// <param name="isstdout">
        ///     if set to <c>true</c> the stream is assumed to be standard output, otherwise assumed to be
        ///     standard error.
        /// </param>
        ...
        private void NotifyAndFlushBufferText(StringBuilder textbuffer, bool isstdout) {
            if (textbuffer.Length > 0) {
                if (isstdout && StdoutTextRead != null) { // Send notificatin of text read from stdout
                    StdoutTextRead(textbuffer.ToString());
                } else if (isstdout == false && StderrTextRead != null) { // Send notificatin of text read from stderr
        ...
        private void ReadStandardOutputThreadMethod() {
            ...
            try {
                ...
                Console.WriteLine("ProcessIoManager.ReadStandardOutputThreadMethod()- Exception caught:" + ex.Message + "\nStack Trace:" + ex.StackTrace);
            }
        }
        ...
        private void ReadStandardErrorThreadMethod() {
            try {
                ...
                Console.WriteLine("ProcessIoManager.ReadStandardErrorThreadMethod()- Exception caught:" + ex.Message + "\nStack Trace:" + ex.StackTrace);
            }
        }
